...
Obs: Após instalar os plugins, acesse configuracoes e busque por on save e habilite Editor: Format On Save para toda vez que salvar o código ser formatado.
...
Digamos que voce esteja codando parte no Linux e parte no Windows, ou codando com outro Dev. 
Entao por exemplo o encerrar da linha, ele é diferente pra cada sistema op, mas os sistemas suportam uns dos outros, entao voce seta isso no editor config e fica padrao. 
...
Com typescript voce vai conseguir utilizar classes, parthers, solid..
...
// "yarn add -D ts-node-dev" serve para adicionar a ferramenta "ts-node-dev" como uma dependência de desenvolvimento que reinicia automaticamente o servidor durante o desenvolvimento de projetos Node.js com TypeScript. Isso agiliza a detecção de alterações no código e recarrega o aplicativo, tornando o desenvolvimento mais eficiente.
...
Obs: Criamos um arquivo .env.local onde ficará salvo as variaveis de ambiente que por enquanto criamos somente variavel abaixo:

PROJECT_NAME='Hello API Env'

Obs: Depois duplicaremos o arquivo .env.local e mudaremos o nome para .env

---------------------------------------------------------------------
Avisando o projeto que ele deve usar o .env:

Execute o comando: yarn add -D dotenv

Depois disso será necessário conifgurar mais algumas coisas no package.json em scripts/dev mude para: ts-node-dev -r dotenv/config ./src/server.ts

Isso evita que quando o nome do Projeto é usado varias vezes no projeto, dentro de uma variavel de ambiente voce consegue usar facilmente.

---------------------------------------------------------------------
Configurando o build e start em produçao:

Obs: No arquivo package.json em scripts adicione "build": "tsc"
...
--------------------------------------------------------------------
Configurando .gitgnore :

gitignore nao deixa arquivos especificos serem enviados para o respositorio no git:
